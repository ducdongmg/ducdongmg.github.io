---
layout: single
title:  "Link useful for selenium, selenide, appium"
desc: "Link useful for selenium, selenide, appium"
excerpt: "Link useful for selenium, selenide, appium"
keywords: "selenium, selenide, appium"
categories: [selenium]
tag: [selenium]
---

Dưới đây là một vài link hữu ích

### Selenide
 - [Introduce & tips](http://selenide-recipes.blogspot.com/2015/08/1-introduction.html)
 - [Setup chromedriver](https://sites.google.com/a/chromium.org/chromedriver/getting-started)
 - [Selenide cheat sheet](https://gist.github.com/mkpythonanywhereblog/947633ba1bf0bc239639)


### Appium

 - [Start server from Java](http://www.automationtestinghub.com/3-ways-to-start-appium-server-from-java/)
 - [Start server from Java 2](https://github.com/appium/java-client/blob/master/docs/The-starting-of-an-app-using-Appium-node-server-started-programmatically.md#fyi)
  - [Running multiple Appium Server for parallel execution](https://nishantverma.gitbooks.io/appium-for-android/running_multiple_appium_server_for_parallel_execut.html)

#### DesiredCapabilities
 - [List option](http://appium.io/docs/en/writing-running-appium/caps/)
 - [Appium Architecture](https://nishantverma.gitbooks.io/appium-for-android/appium/why_appium.html)

 